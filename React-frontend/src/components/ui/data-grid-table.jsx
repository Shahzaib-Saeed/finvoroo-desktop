...
function DataGridTableFoot({ children }) {
  ...
  return (
    <tfoot className={cn('bg-muted/30', props.tableClassNames?.footer)}>
      {children}
    </tfoot>
  );
}

function DataGridTableFooterRow({ children }) {
  const { props } = useDataGrid();
  ...
  return (
    <tr
      className={cn(
        ...
        props.tableLayout?.cellBorder && '[&_>:last-child]:border-e-0',
        props.tableClassNames?.footerRow,
      )}
    >
      {children}
    </tr>
...
function DataGridTableFooterCell({ children, column, className }) {
  const { props, table } = useDataGrid();
  const bodyCellSpacing = bodyCellSpacingVariants({
    size: props.tableLayout?.dense ? 'dense' : 'default',
  });
...
function DataGridTableRowSelectAll({ size }) {
  ...
  return (
    <Checkbox
      ...
      disabled={isLoading || recordCount === 0 || !hasSelectable}
...
function DataGridTable() {
  ...
  return (
    <DataGridTableBase>
      ...
      {props.tableFooter ? props.tableFooter : null}
    </DataGridTableBase>
  );
}
