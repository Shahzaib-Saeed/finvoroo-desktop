...
export function VendorDetailsAttachments({ vendorId, attachments = [] }) {
  const [busyId, setBusyId] = useState(null);
  const { previewSavedAttachment, previewProps } = useDocumentAttachmentPreview();

  const handleDownload = useCallback(
    async (attachment) => {
      if (!vendorId || !attachment?.id) return;
  ...
  const handleView = useCallback(
    async (attachment) => {
      if (!vendorId || !attachment?.id) return;
      await previewSavedAttachment(vendorsApi, vendorId, attachment, () =>
        handleDownload(attachment),
      );
    },
    [vendorId, previewSavedAttachment, handleDownload],
  );
