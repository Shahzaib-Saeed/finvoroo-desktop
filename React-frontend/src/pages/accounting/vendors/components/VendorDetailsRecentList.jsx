...
export function VendorDetailsRecentList({
  title,
  icon: Icon,
  rows = [],
  emptyMessage,
...
export function VendorDetailsRecentPurchaseOrders({ orders, workspaceId, tabLink, onViewAll }) {
  const recent = orders.slice(0, 5);
  const base = `/workspace/${workspaceId}/accounting/purchase-orders`;

  return (
    <VendorDetailsRecentList
      ...
      renderPrimary={(row) => row.po_number || `PO-${row.id}`}
...
export function VendorDetailsRecentPayments({ payments, workspaceId, tabLink, onViewAll }) {
  ...
  return (
    <VendorDetailsRecentList
      ...
      onViewAll={onViewAll}
      getHref={() => base}
      renderPrimary={(row) => row.payment_number || row.reference || '—'}
      renderSecondary={(row) =>
