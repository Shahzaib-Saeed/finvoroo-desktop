...
export function StockAdjustmentEditPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    Promise.all([stockAdjustmentsApi.formOptions(), stockAdjustmentsApi.show(adjustmentId)])
      .then(([optRes, showRes]) => {
        if (cancelled) return;
        const data = optRes.data?.data || {};
        const adj = showRes.data?.data;
        if (!adj) {
          toast.error('Stock adjustment not found');
          navigate(base);
          return;
        }
        if (!adj.flags?.can_edit) {
          toast.error('This stock adjustment cannot be edited');
          navigate(`${base}/${adjustmentId}`);
          return;
        }
        setOpts({
        ...
        const mapped = formFromAdjustment(adj);
        setForm({
          ...mapped,
          inventory_account_id: adj.journal_entry_id
            ? mapped.inventory_account_id
            : d.inventory_account_id
              ? String(d.inventory_account_id)
              : mapped.inventory_account_id,
          adjustment_expense_account_id: d.adjustment_expense_account_id
            ? String(d.adjustment_expense_account_id)
            : mapped.adjustment_expense_account_id,
          adjustment_income_account_id: d.adjustment_income_account_id
            ? String(d.adjustment_income_account_id)
            : mapped.adjustment_income_account_id,
        });
      })
      .catch((err) => {
        if (!cancelled) {
          toast.error(err?.response?.data?.message || 'Failed to load adjustment');
          navigate(base);
        }
      })
      .finally(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    setErrors({});
    try {
      const res = await stockAdjustmentsApi.update(adjustmentId, buildAdjustmentPayload(form));
      toast.success(res.data?.message || 'Adjustment updated');
      navigate(`${base}/${adjustmentId}`);
    } catch (err) {
  ...
  return (
    <div className="space-y-6 w-full min-w-0">
      <PageHeader
        ...
        subtitle="Changes safely unwind and rebuild the ledger and GL journal, even for a posted adjustment."
        actions={
          <Button variant="outline" size="sm" asChild>
            <Link to={`${base}/${adjustmentId}`}>
      ...
      >
        ...
        <div className="flex justify-end gap-2">
          <Button type="button" variant="outline" onClick={() => navigate(`${base}/${adjustmentId}`)} disabled={saving}>
            Cancel
          </Button>
