...
import { INCOME_STATEMENT_COLUMNS } from "./constants/report-columns";
...
function downloadIncomeStatementCsv({
  ...
  for (const [section, rows] of pairs) {
  ...
  out.push([
    ...
    "Total expenses",
    formatAmountCsv(
      (summary.total_operating_expenses ?? 0) + (summary.total_other_expenses ?? 0),
    ),
  ]);
...
export function IncomeStatementReportPage() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    reportsApi
      .incomeStatement({ from: period.from, to: period.to })
      .then((res) => setData(res.data?.data || null))
      .catch((err) =>
        toast.error(
          err?.response?.data?.message || "Failed to load income statement",
        ),
      )
  ...
  const reportFilename = useMemo(
    () => buildReportFilename("income-statement", companyName, periodEnd),
    ...
    async (mode) => {
      ...
      try {
        await (mode === "pdf" ? downloadReportPdf : printReportSheet)(node, {
          ...
          rootClass: "income-statement-report-root",
        });
        if (mode === "pdf") {
  ...
  const handleExport = useCallback(() => {
    ...
    downloadIncomeStatementCsv({
      filename: `income-statement-${safeName || "report"}-${periodEnd || "export"}.csv`,
      companyName,
  ...
  return (
    <ReportPageShell
      ...
      subtitle="Revenue, cost of sales, expenses, and net income for the selected period."
      actions={
        <ReportActionBar
          leading={
            <ReportTableToolbar
      ...
      contentClassName="w-full max-w-[1600px] mx-auto space-y-4 income-statement-report-root"
    >
      ...
      {loading && !data ? (
        ...
        <div
          ref={sheetRef}
          className="report-print-sheet income-statement-print overflow-visible rounded-lg border border-slate-200 bg-white print:overflow-visible print:rounded-none print:border-0"
        >
          <ProfitLossStatement
